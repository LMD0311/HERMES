...
def _make_causal_mask(
    ...
    img_mask = (img_select * img_select.view(mask.size(-1), 1)).to(device)
    mask.masked_fill_(img_mask, 0)
...
class InternLM2FlashAttention2(InternLM2Attention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        qkv_states = self.wqkv(hidden_states)  ## TO DO: checkpoint
        ...
        attn_output = self.wo(attn_output)  ## TO DO: checkpoint
...
class InternLM2Model(InternLM2PreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length,
        ...
        # mask = np.asarray(combined_attention_mask[0][0].to(torch.float32).to('cpu'))
        # mask -= np.min(mask)
