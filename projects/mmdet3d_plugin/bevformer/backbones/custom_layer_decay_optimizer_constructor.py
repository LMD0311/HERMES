...
@OPTIMIZER_BUILDERS.register_module()
class CustomDecayOptimizerConstructor(DefaultOptimizerConstructor):
    ...
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        logger = get_root_logger()
        logger.info(self.paramwise_cfg)
        lr_scale_cfg = self.paramwise_cfg.get('custom_keys', {})
        weight_decay = self.base_wd
        ...
        for name, param in module.named_parameters():
            ...
            full_param_name = prefix + ('.' if prefix else '') + name
            ...
            for key in lr_scale_cfg:
                if (full_param_name.startswith(key) or
                        (prefix and prefix.startswith(key))):
                    scale = lr_scale_cfg[key]['lr_mult']
                    matched_key = key
                    break
            ...
            if group_name not in parameter_groups:
                ...
                if parameter_groups[group_name]["lr_scale"] != scale:
                    new_group_name = f"{group_name}_{scale}"
                    if new_group_name not in parameter_groups:
                        parameter_groups[new_group_name] = {
                            "weight_decay": this_weight_decay,
                            "params": [],
                            "param_names": [],
                            "lr_scale": scale,
            ...
            parameter_groups[group_name]["params"].append(param)
            parameter_groups[group_name]["param_names"].append(full_param_name)
        ...
        if rank == 0:
            ...
            for key in parameter_groups:
                to_display[key] = {
                    "param_names": parameter_groups[key]["param_names"][:5] +
                                   (["..."] if len(parameter_groups[key]["param_names"]) > 5 else []),
                    "param_count": len(parameter_groups[key]["param_names"]),
                    "lr_scale": parameter_groups[key]["lr_scale"],
                    "lr": parameter_groups[key]["lr"],
                    "weight_decay": parameter_groups[key]["weight_decay"],
                    "matched_key": parameter_groups[key].get("matched_key", None),
                }
            logger.info("Param groups = %s" % json.dumps(to_display, indent=2))
